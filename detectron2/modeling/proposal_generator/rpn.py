...
@RPN_HEAD_REGISTRY.register()
class GatedMultiRPNHead(nn.Module):
    """
    Standard RPN classification and regression heads described in :paper:`Faster R-CNN`.
    Uses a 3x3 conv to produce a shared hidden state from which one 1x1 conv predicts
    objectness logits for each anchor and a second 1x1 conv predicts bounding-box deltas
    ...
    @configurable
    def __init__(
        self, *, in_channels: int, num_anchors: int, box_dim: int = 4, conv_dims: List[int] = (-1,), gate_num: int = 1
    ):
        """
        NOTE: this interface is experimental.
        ...
        if len(conv_dims) == 1:
            for _ in range(gate_num):
                out_channels = cur_channels if conv_dims[0] == -1 else conv_dims[0]
                # 3x3 conv for the hidden representation
                self.conv.append(self._get_rpn_conv(cur_channels, out_channels)
                                 )
                cur_channels = out_channels
        else:
            for _ in range(gate_num):
                ...
                for k, conv_dim in enumerate(conv_dims):
                    ...
                    conv_block.add_module(f"conv{k}", conv)
                    cur_channels = out_channels
                self.conv.append(conv_block)
        self.conv = nn.ModuleList(self.conv)
        # 1x1 conv for predicting objectness logits
        self.objectness_logits = [
            nn.Conv2d(cur_channels, num_anchors, kernel_size=1, stride=1) for _ in range(gate_num)]
        # 1x1 conv for predicting box2box transform deltas
        self.anchor_deltas = [nn.Conv2d(cur_channels, num_anchors
                                        * box_dim, kernel_size=1, stride=1) for _ in range(gate_num)]

        self.objectness_logits = nn.ModuleList(self.objectness_logits)
        self.anchor_deltas = nn.ModuleList(self.anchor_deltas)
